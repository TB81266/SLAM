       ...
       void updateX()
        ...
        void solveByGaussNewton()
        {
            ...
            for( size_t i = 0; i < max_iter_; i ++)
            {
                ...
                if( is_out_ )
                {
                cout << "Iter: " << left <<setw(3) << i << " Result: "<< left <<setw(10)  << *a_ << " " << left; 
		        cout<<setw(10)  << *b_ << " " << left <<setw(10) << *c_ <<  " step: " << left <<setw(14) << delta;
		        cout<< " cost: "<< left <<setw(14)  << getCost() << " time: " << left;
		        cout<<setw(14) << t.duration()  <<" total_time: "<< left <<setw(14) << (sumt += t.duration()) << endl;
                }
                if( delta < min_step_)
                {
                    is_conv = true;
